...
<Danger>
  Kinsta Enhancer uses a **company-level Kinsta API key** and therefore **every site and environment under that Kinsta account** is available in the settings UI.

  This plugin should only be used under the following scenarios:

  - Installed on the Kinsta account holder's website only.
